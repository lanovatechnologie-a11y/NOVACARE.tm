...
// Fonction pour générer le prochain numéro de ticket unique
async function getNextTicketNumber() {
  try {
    // Trouver le plus grand numéro dans la collection Ticket
    const lastTicket = await Ticket.findOne().sort({ number: -1 });
    
    if (!lastTicket) {
      // Si aucun ticket n'existe, commencer à 100001
      return 100001;
    }
    
    // Vérifier s'il y a des "trous" dans la séquence (tickets supprimés)
    // Pour être sûr, on prend le plus grand numéro + 1
    const maxNumber = lastTicket.number;
    
    // Vérifier si le numéro suivant existe déjà (peu probable avec unique index)
    const nextNumber = maxNumber + 1;
    const existing = await Ticket.findOne({ number: nextNumber });
    
    if (existing) {
      // Trouver le prochain numéro disponible
      const tickets = await Ticket.find().sort({ number: -1 }).limit(100);
      const usedNumbers = new Set(tickets.map(t => t.number));
    ...
    // En cas d'erreur, retourner un numéro basé sur le timestamp
    return Math.floor(100001 + Date.now() % 100000);
  }
}

// Fonction pour générer le prochain numéro de ticket multi-tirages
async function getNextMultiDrawTicketNumber() {
  try {
    const lastTicket = await MultiDrawTicket.findOne().sort({ number: -1 });
    
    if (!lastTicket) {
      // Si aucun ticket multi-tirages n'existe, commencer à 500001
      return 500001;
    }
    
    return lastTicket.number + 1;
  } catch (error) {
    console.error('Erreur génération numéro ticket multi-tirages:', error);
    return Math.floor(500001 + Date.now() % 100000);
  }
}
...
// =================== ROUTE CORRIGÉE : Sauvegarder un ticket ===================
app.post('/api/tickets', vérifierToken, async (req, res) => {
    try {
        ...
        console.log('📥 Données reçues pour ticket:', {
            ticketNumber, draw, draw_time, total,
        ...
        // CORRECTION CRITIQUE : Générer un nouveau numéro de ticket unique
        ...
        // Si c'est une erreur de duplication de clé
        if (error.code === 11000) {
            // Régénérer un nouveau numéro et réessayer
            console.log('❌ Erreur de duplication, tentative de régénération...');
            return res.status(500).json({
                success: false,
                error: 'Erreur de duplication de numéro de ticket. Veuillez réessayer.'
            });
        }
...
app.post('/api/tickets/pending', vérifierToken, async (req, res) => {
  try {
    ...
    // Générer un nouveau numéro de ticket unique
...
app.post('/api/tickets/multi-draw', vérifierToken, async (req, res) => {
  try {
    ...
    // Générer un nouveau numéro de ticket multi-tirages unique
    const ticketNumber = await getNextMultiDrawTicketNumber();
...
app.listen(PORT, () => {
  ...
  console.log('  POST   /api/tickets                     - Sauvegarder ticket (CORRIGÉ)');
  ...
  console.log('');
  console.log('⚠️  IMPORTANT: Les numéros de tickets sont maintenant générés automatiquement par le serveur');
  console.log('   pour garantir l\'unicité et éviter les duplications.');
  console.log('');
