...
// FONCTION findPatient CORRIGÉE (de lolo.html)
...
// FONCTIONS SETTINGS CORRIGÉES (de lolo.html)
function setupSettings() {
    ...
    if(btnLab) {
        btnLab.addEventListener('click', function(e) {
            ...
            state.labAnalyses.push({ id: Date.now(), name: name, price: price, active: true });
            nameInput.value = '';
            priceInput.value = '';
            updateSettingsDisplay();
...
function updateSettingsDisplay() {
    ...
    state.consultationTypes.forEach(type => {
        if(type.active) {
            consultationList.innerHTML += `<div class="setting-item"><strong>${type.name}</strong> <span class="service-price">${type.price} Gdes</span></div>`;
        }
    });
    ...
    state.externalServices.forEach(service => {
        if(service.active) {
            externalServicesList.innerHTML += `<div class="setting-item"><strong>${service.name}</strong> <span class="service-price">${service.price} Gdes</span></div>`;
        }
    });
    ...
    state.labAnalyses.forEach(analysis => {
        if(analysis.active) {
            labAnalysesList.innerHTML += `<div class="setting-item"><strong>${analysis.name}</strong> <span class="service-price">${analysis.price} Gdes</span></div>`;
        }
    });
}
